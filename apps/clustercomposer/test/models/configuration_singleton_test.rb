...
class ConfigurationSingletonTest < ActiveSupport::TestCase
  ...
  test "loading custom OSC external config in production env" do
    ...
    assert_equal File.expand_path("~/ondemand/data/sys/clustercomposer"), config.dataroot.to_s
    assert_equal File.expand_path("~/ondemand/data/sys/clustercomposer/production.sqlite3"), config.production_database_path.to_s
  ...
  test "loading custom AweSim external config in production env" do
    ...
    assert_equal File.expand_path("~/awesim/data/sys/clustercomposer"), config.dataroot.to_s
    assert_equal File.expand_path("~/awesim/data/sys/clustercomposer/production.sqlite3"), config.production_database_path.to_s
    assert_equal true, config.load_external_config
  end
  ...
  test "setting dataroot and database path" do
    Bundler.with_clean_env do
      ...
      # default
      assert_equal File.expand_path("~/ondemand/data/sys/clustercomposer"), ConfigurationSingleton.new.dataroot.to_s
      assert_equal File.expand_path("~/ondemand/data/sys/clustercomposer/production.sqlite3"), ConfigurationSingleton.new.production_database_path.to_s

      # set to tmp dir
      ENV['OOD_DATAROOT'] = "/tmp/clustercomposer"
      assert_equal "/tmp/clustercomposer", ConfigurationSingleton.new.dataroot.to_s
      assert_equal "/tmp/clustercomposer/production.sqlite3", ConfigurationSingleton.new.production_database_path.to_s
