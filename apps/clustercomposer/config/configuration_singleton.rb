...
class ConfigurationSingleton
  ...
  def config_root
    Pathname.new(ENV["OOD_APP_CONFIG_ROOT"] || "/etc/ood/config/apps/clustercomposer")
  end
  ...
  # Custom cluster templates source directory in /etc/.../clustercomposer/templates
...
#  def show_cluster_options_account_field?
...
#  end
  ...
  # files. The default for OOD_APP_CONFIG_ROOT is /etc/ood/config/apps/clustercomposer and
  ...
  def dataroot
    ...
    if rails_env == "production"
      root ||= "~/#{ENV['OOD_PORTAL'] || 'ondemand'}/data/#{ENV['APP_TOKEN'] || 'sys/clustercomposer'}"
    else
