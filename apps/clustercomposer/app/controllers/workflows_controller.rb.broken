class WorkflowsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @workflow.save
        ...
        format.html { redirect_to workflows_url, notice: 'Cluster was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @workflow.update(workflow_params)
        session[:selected_id] = @workflow.id
        format.html { redirect_to workflows_path, notice: 'Cluster was successfully updated.' }
  ...
  def stop
    ...
    respond_to do |format|
      if !@workflow.submitted?
        format.html { redirect_to workflows_url, alert: 'Cluster has not been submitted.' }
        format.json { head :no_content }
      elsif @workflow.stop
        format.html { redirect_to workflows_url, notice: 'Cluster was successfully stopped.' }
        format.js   { render :show }
        format.json { head :no_content }
      else
        @errors = @workflow.errors
        format.html { redirect_to workflows_url, alert: "Cluster failed to be stopped: #{@workflow.errors.to_a}" }
  ...
  def destroy
    ...
    respond_to do |format|
      if @workflow.destroy
        format.html { redirect_to workflows_url, notice: 'Cluster was successfully destroyed.' }
        format.json { head :no_content }
      else
        format.html { redirect_to workflows_url, alert: "Cluster failed to be destroyed: #{@workflow.errors.to_a}" }
        format.json { render json: @workflow.errors, status: :internal_server_error }
      end
    end
  end
  ...
  def submit
    ...
    respond_to do |format|
      if @workflow.submitted?
        session[:selected_id] = @workflow.id
        format.html { redirect_to workflows_url, alert: 'Cluster has already been submitted.' }
        format.json { head :no_content }
      elsif @workflow.submit
        session[:selected_id] = @workflow.id
        format.html { redirect_to workflows_url, notice: 'Cluster was successfully submitted.' }
        format.json { head :no_content }
      else
  ...
  def copy
    ...
    respond_to do |format|
      if @workflow.errors.empty? && @workflow.save
        session[:selected_id] = @workflow.id
        format.html { redirect_to workflows_url, notice: 'Cluster was successfully copied.' }
        format.json { render :show, status: :created, location: @workflow }
      else
        format.html { redirect_to workflows_url, alert: "Cluster failed to be copied: #{@workflow.errors.to_a}" }
        format.json { render json: @workflow.errors, status: :unprocessable_entity }
      end
    end
  end
