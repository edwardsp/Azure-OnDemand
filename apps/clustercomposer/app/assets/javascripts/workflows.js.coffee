...
$(window).focus ->
  if active_var()
    request_cluster_data(active_var())
  return

# This method is called when a user selects a cluster.
@update_display = (id) ->
  disable_all_buttons()
  update_script_details_panel()
  request_cluster_data(id)
...
@update_cluster_panel = (id, data) ->
  ...
  update_cluster_details_panel(data)
...
@show_cluster_panel = (show) ->
  if show?
    $("#cluster-details-panel").show()
  else
    $("#cluster-details-panel").hide()
...
@show_cluster_array_request = (show) ->
  if show
    $("#cluster-details-cluster-array-request").show()
    $("#cluster-details-cluster-array-request").prev().show()
  else
    $("#cluster-details-cluster-array-request").hide()
    $("#cluster-details-cluster-array-request").prev().hide()
...
@update_cluster_details_panel = (data) ->
  ...
  if data?
    ...
    if(data.xdmod_url)
      ...
      $("#cluster-details-id").html('<a target="_blank" href="'+data.xdmod_url+'">'+data.pbsid+' - <i style="display: inline" class="fa fa-external-link-square-alt"></i>&nbsp;XDMoD</a>')
    else
      $("#cluster-details-id").text(data.pbsid)
    $("#cluster-details-name").text(data.name)
    $("#cluster-details-server").val(data.host_title)
    $("#cluster-details-staged-dir").text(data.staged_dir)
    $("#cluster-details-script-name").text(data.staged_script_name || ' ')
    $("#cluster-details-cluster-array-request").val(data.cluster_array_request || '')
    show_cluster_array_request( !(data.cluster_array_request == null or data.cluster_array_request == '') )
    if data.account == null or data.account == ""
      $("#cluster-details-account").addClass("text-muted")
      $("#cluster-details-account").text("Not specified")
    else
      $("#cluster-details-account").removeClass("text-muted")
      $("#cluster-details-account").text(data.account)
...
@list_folder_contents = (data) ->
  ...
  if data?
    ...
    list += "</ul>"
    $("#cluster-details-staged-dir-contents").html(list)
  else
    $("#cluster-details-staged-dir-contents").html("")
  update_script_details_panel(submit_script)

$ ->
  $('#new_cluster_template_selectpicker').on 'change', ->
...
@update_new_cluster_display = (row) ->
  if row?
...
@update_missing_data_cluster_view = ->
  $('#cluster-details-submit-to').toggleClass("missing-cluster", missing_data_cluster())
  $("#edit-cluster-options-script-link").attr("href", Routes.edit_workflow_path(active_var()))
...
@update_missing_data_script_view = ->
  ...
  $("#edit-cluster-options-link").attr("href", Routes.edit_workflow_path(active_var()))

$ ->
  $('#reset-template-data').on 'click', ->
    update_new_cluster_display(active_row())
